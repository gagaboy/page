...
(function(){
    ...
    var SynSend = function(){
    	...
    	oThis.sendAppPageData = function(pageData){
    		...
    		// console.log(val);
    	    // var url =
			// embUrl+"?appFrontCookieValue="+encodeURI(encodeURI(val))+"&callback=appDataSendStatus";
    		var url = embUrl+"?secret=iojawdnaisdflknoiankjfdblaidcas&apiType=produce&token=iojawdnaisdflknoiankjfdblaidcas&contentType=text/plain"+"&content="+encodeURI(encodeURI(val))+"&callback=appDataSendStatus"+"&timeFlag="+new Date().getTime();
    		// console.log(url);
	...
	// 浏览器窗口关闭时候，会发送2个页面结束的异步请求，2个请求中页面结束时间相差几微秒，以小的值作为页面结束时间
    // console.log("不是safari处理");
...
// //Safari通过鼠标互动点击，来确定页面停留时间(如果页面没有鼠标点击，而是直接关闭，则获取不到页面停留时间)
// if(isSafari){
// //console.log("safari处理");
...
// clickNumArry.length=0;
// pageData =
// curTitle+"YzY"+curUrl+"YzY"+startTime+"YzY"+endTime+"YzY"+refUrl+"YzY"+clickNumStr;
// }else{
// pageData = curTitle+"YzY"+curUrl+"YzY"+startTime+"YzY"+endTime+"YzY"+refUrl;
// }
// _synSend.sendAppPageData(pageData);
